...
The encrypted plaintext is a small **content envelope** `{ ts, ct, x }` — the sender's send time (`ts`, epoch ms), a content type (`ct`, one of `text` or `image`), and the content (`x`). For `text`, `x` is the message string; for `image`, `x` is a **base64 `data:` URL** of a JPEG that the sender **resizes and compresses on-device** (bounded longest edge + quality) before sealing, so the payload stays small. An image is sealed **exactly like text** — the Server sees only ciphertext and stores the one opaque `payload` (single-copy for 1:1 and group alike); the content type lives **inside** the encrypted envelope, so no server, DTO, or database change is needed. The recipient decrypts and renders the image inline. Every device orders a message by `ts` rather than its own arrival time, so the timeline is identical on all devices. The same content envelope is used for group messages.

> **Payload-size guard.** Because a message body is opaque to the Server, it cannot inspect content but it does cap the raw `payload` length (rejecting oversized bodies with `400`) as a resource-exhaustion safeguard. The client's image resize/compress keeps a photo well under that cap; a message exceeding it is refused rather than stored.
