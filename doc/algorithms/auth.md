...
- **Client reaction to `revoked`.** On a `revoked` refresh response the client **wipes all local Signal state** (identity, pre-keys, sessions) but keeps the messages, conversations, and the DB master key, so local history stays readable, and returns to login. On the subsequent login it finds no Signal identity, regenerates one, and re-uploads the pre-key bundle (`PUT /keys`). See [MESSAGE_SECURITY.md §2.1](../MESSAGE_SECURITY.md#21-key-lifecycle-logout--single-active-device-policy).

- **Explicit logout keeps keys.** A user-initiated logout only deletes the server-side refresh token (by `sessionId`) and clears the client's access token + `sessionId`; the Signal identity and session state remain so re-login on the same device is seamless and does not rotate the directory.