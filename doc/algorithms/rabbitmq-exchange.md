...
- **Holding Queue (`unrouted.holding.queue`):** Buffers messages dropped by the alternate exchange for up to 30 seconds (TTL) without database interaction. This gives the recipient's mobile push notifications a brief window to wake up the app and retrieve the data in-memory.
...
flowchart TD
    ...
    subgraph RabbitMQ [RabbitMQ Internal Broker]
        ...
        HoldingQueue[unrouted.holding.queue<br/>TTL: 30 seconds]
    ...
    AlternateExchange --> HoldingQueue
